...
| What | Path |
|------|------|
| Plugin source (dev) | `/home/erebus/agent/coding-tool/` |
...
**Problem**: openclaw user can't follow symlinks through `/home/erebus/agent/coding-tool`.
**Fix**: Both `/home/erebus` and `/home/erebus/agent` need `o+x` (execute for others). `/home/erebus` has `0701`, `/home/erebus/agent` has `0755`.

### 13. Claude CLI must be copied, not symlinked
**Problem**: nvm-installed Claude CLI lives under `/home/erebus/.nvm/...` — openclaw can't traverse.
**Fix**: Copy the actual binary: `sudo cp $(which claude) /usr/local/bin/claude`. Codex works as a symlink because its nvm path is under the traversable `/home/erebus/agent/.nvm/`.
